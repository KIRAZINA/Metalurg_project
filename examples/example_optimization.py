...
def example_optimization(file_path: str, output_dir: str):
    ...
    s_input, s_feasible, s_conf = inverse_regressor.predict_required_input(
        "steel_S_before", s_target
    )
    si_input, si_feasible, si_conf = inverse_regressor.predict_required_input(
        "steel_Si_before", si_target
    )
    ...
    generate_optimization_report(inverse_regressor, optimization_targets, str(opt_report_path))
    ...
    logging.info(
        f"✓ Pareto front CSV saved to {str(opt_report_path).replace('.csv', '_pareto_front.csv')}"
    )
    if plot_path:
